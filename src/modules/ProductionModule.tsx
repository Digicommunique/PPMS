...
import { 
  ...
  Pause, ChevronRight, User, Cpu, BarChart3, AlertTriangle, X
} from 'lucide-react';
...
export default function ProductionModule() {
  const [jobs, setJobs] = useState<ActiveJob[]>(mockActiveJobs);
                      ...
                      <button 
                        onClick={() => {
                          if (selectedJobId) {
                            ...
                            alert('Batch output initialized for all pending stages.');
                          }
                        }}
                        className="px-6 py-3 bg-primary text-white rounded-xl font-bold flex items-center gap-2 hover:bg-accent-cyan shadow-lg shadow-primary/20 transition-all"
                      >
                          ...
                          <div className="space-y-4 font-sans">
                             ...
                             {stage.status === 'In Progress' ? (
                               <button 
                                 ...
                                 className="w-full mt-4 py-3 bg-accent-cyan hover:bg-accent-cyan-dark text-white rounded-xl font-bold flex items-center justify-center gap-2 shadow-lg shadow-accent-cyan/20 transition-all uppercase tracking-widest text-[10px]"
                               >
                                  <Pause size={14} /> Pause & Record Output
                               </button>
                             ) : stage.status === 'Pending' ? (
                               <button 
