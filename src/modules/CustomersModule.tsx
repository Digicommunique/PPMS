...
export default function CustomersModule() {
  ...
  const handleAddCustomer = (e: React.FormEvent) => {
    ...
    const newCustomer: Customer = {
      ...
      balance: 0,
      type: formData.get('type') as any || 'Corporate',
      creditLimit: Number(formData.get('creditLimit')) || 0,
      paymentTerms: formData.get('paymentTerms') as string || 'Due on Receipt',
      joinedDate: new Date().toISOString().split('T')[0],
      contacts: [{ name: formData.get('name') as string, phone: formData.get('phone') as string, email: formData.get('email') as string, designation: 'Primary' }],
      addresses: [],
      history: []
    };
                  ...
                  {customers.filter(c => 
                    ...
                    >
                      ...
                      <td className="px-6 py-4 text-right">
                        <button className="p-2 hover:bg-white rounded-full text-secondary transition-colors shadow-sm bg-gray-50">
          ...
          {selectedCustomerId && selectedCustomer && (
            ...
            >
              <div className="bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden sticky top-6">
                <div className="p-6 bg-gradient-to-br from-primary to-primary-dark text-white relative overflow-hidden">
                  <button 
                ...
                <div className="p-6 space-y-8">
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center justify-between">
                       ...
                       <button className="text-[10px] font-bold text-accent-cyan hover:underline">+ Add Contact</button>
                    </div>
                      ...
                      {selectedCustomer.contacts.map((contact, idx) => (
                        <div key={idx} className="p-3 border border-gray-100 rounded-xl hover:border-accent-cyan/30 transition-all bg-gray-50/30 group">
                           ...
                           <div className="mt-2 space-y-1">
                              <p className="text-xs text-secondary flex items-center gap-2"><Phone size={10} /> {contact.phone}</p>
                              <p className="text-xs text-secondary flex items-center gap-2"><Mail size={10} /> {contact.email}</p>
                           </div>
                        </div>
                      ))}
                    </div>
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center justify-between">
                       ...
                       <button className="text-[10px] font-bold text-accent-magenta hover:underline">+ Add Address</button>
                    </div>
                    <div className="space-y-3">
                      {selectedCustomer.addresses.map((addr, idx) => (
                        <div key={idx} className="flex gap-3 text-xs">
                          <div className={`p-2 rounded-lg h-fit ${addr.type === 'Billing' ? 'bg-accent-amber/10 text-accent-amber' : 'bg-success/10 text-success'}`}>
                             {addr.type === 'Billing' ? <CreditCard size={14} /> : <MapPin size={14} />}
                          </div>
                          <div>
                            <p className="font-bold text-primary">{addr.type} Address</p>
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center justify-between">
                       ...
                       <button className="text-[10px] font-bold text-accent-amber hover:underline">Log Entry</button>
                    </div>
                    <div className="space-y-4 relative before:absolute before:left-[11px] before:top-2 before:bottom-2 before:w-[1px] before:bg-gray-100">
                      {selectedCustomer.history.length > 0 ? selectedCustomer.history.map((log) => (
                        <div key={log.id} className="relative pl-8">
                           <div className="absolute left-0 top-1 w-6 h-6 rounded-full border-4 border-white bg-gray-50 flex items-center justify-center shadow-sm">
                  ...
                  <div className="pt-6 border-t border-gray-100 flex gap-3">
                    <button className="flex-1 px-4 py-3 bg-white border border-gray-200 rounded-xl font-bold text-sm hover:bg-gray-50 transition-all flex items-center justify-center gap-2 group">
                       <MessageSquare size={16} className="text-accent-cyan group-hover:scale-110 transition-transform" /> Chat
                    </button>
                    <button className="flex-1 px-4 py-3 bg-primary text-white rounded-xl font-bold text-sm hover:bg-primary-dark transition-all flex items-center justify-center gap-2 shadow-lg shadow-primary/20 group">
                       <Calculator size={16} className="group-hover:rotate-12 transition-transform" /> Quote
                    </button>
                  </div>
                </div>
              </div>
            ...
            >
              ...
              <form onSubmit={handleAddCustomer} className="p-8 space-y-6 max-h-[80vh] overflow-y-auto custom-scrollbar">
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div className="space-y-2">
                      <label className="text-xs font-bold text-primary uppercase tracking-wider flex items-center gap-2">
                        <ShieldCheck size={14} className="text-success" /> GST Number
                      </label>
                      <input name="gst" required placeholder="27XXXXX0000X1Z5" className="w-full px-4 py-2 bg-gray-50 border border-gray-100 rounded-lg outline-none focus:ring-2 focus:ring-accent-cyan transition-all" />
                    </div>
                    <div className="space-y-2">
                      <label className="text-xs font-bold text-primary uppercase tracking-wider">Business Type</label>
                      <select name="type" className="w-full px-4 py-2 bg-gray-50 border border-gray-100 rounded-lg outline-none focus:ring-2 focus:ring-accent-cyan">
                ...
                <div className="space-y-4 pt-2">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <input name="phone" required placeholder="+91 99999 88888" className="w-full px-4 py-2 bg-gray-50 border border-gray-100 rounded-lg outline-none focus:ring-2 focus:ring-accent-cyan transition-all" />
                    </div>
                    <div className="col-span-2 space-y-2">
                       <label className="text-xs font-bold text-primary uppercase tracking-wider flex items-center gap-2">
                ...
                <div className="pt-6 flex gap-4">
                  <button type="button" onClick={() => setIsModalOpen(false)} className="flex-1 px-4 py-3 border-2 border-gray-100 rounded-xl font-bold text-secondary hover:bg-gray-50 transition-all font-sans uppercase tracking-widest text-xs">
                    Cancel
                  </button>
                  <button type="submit" className="flex-2 px-4 py-3 bg-primary text-white rounded-xl font-bold hover:bg-accent-cyan shadow-xl shadow-primary/20 transition-all font-sans uppercase tracking-widest text-xs">
